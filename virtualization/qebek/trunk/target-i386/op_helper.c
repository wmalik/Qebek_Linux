...
/* For QEBEK, added by Paulo */
target_ulong* find_system_call_table_address(){
	int intno = 0x80; //interrupt 80 for system calls

	SegmentCache *dt;
	target_ulong idt_descriptor_ptr;
	uint32_t e1, e2, syscall;

	dt = &env->idt;
	idt_descriptor_ptr = dt->base + intno * 8;
	e1 = ldl_kernel(idt_descriptor_ptr);
	e2 = ldl_kernel(idt_descriptor_ptr + 4);
	syscall = (e2 & 0xffff0000) | (e1 & 0x0000ffff);

	target_ulong* sys_call_table = NULL;

	int i;
	for (i=0; i<500; i++) {
		unsigned char* tripleByte = qemu_malloc(3);
		qebek_read_raw(env, syscall+i, tripleByte, 3);

		if(tripleByte[0] == 0xff && tripleByte[1] == 0x14 && tripleByte[2] == 0x85){
			sys_call_table = qemu_malloc(sizeof(target_ulong));
			qebek_read_ulong(env, syscall+i+3, sys_call_table);
			//fprintf(stderr, "sys_call_table address=0x%x\n", *sys_call_table);
		}
		qemu_free(tripleByte);
	}
...
{
	fprintf(stderr, "helper_syscall1\n");
...
void helper_syscall(int next_eip_addend)
{
	fprintf(stderr, "helper_syscall2\n");
	int selector;
		...
		target_ulong next_eip, int is_hw)
{
	if(intno == 0x80) {
		...
		if(unlikely(qebek_syscall_init == 0)){
			target_ulong* sys_call_table_base = find_system_call_table_address();
			qebek_hook_syscall(env, sys_call_table_base);
		}
	}
...
void helper_sysenter(void)
{
	...
	fprintf(stderr, "helper_sysenter\n");
