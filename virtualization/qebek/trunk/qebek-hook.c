...
#include "qebek-linux-console.h"
#include "qebek-linux-console.c"

int qebek_syscall_init = 0;


//FIXME: Only works for linux, generalize for windows
void qebek_hook_syscall(CPUState *env, target_ulong* sys_call_table_base)
{
	if(sys_call_table_base == NULL){
		fprintf(stderr, "qebek_hook_syscall: Failed to retrieve system call table. Not hooking any system calls.\n");
		qebek_syscall_init = True;
		return;
	} else {
		fprintf(stderr, "qebek_hook_syscall: Successfully found system call table address at: 0x%x\n", *sys_call_table_base);
	}

	qebek_read_ulong(env, *sys_call_table_base + INDEX_LINUX_READ * 4, &LinuxRead);
	qebek_read_ulong(env, *sys_call_table_base + INDEX_LINUX_WRITE * 4, &LinuxWrite);

	if(!qebek_bp_add(LinuxRead, 0, 0, preLinuxRead, NULL))
	{
		fprintf(stderr, "qebek_hook_syscall: failed to insert break point for preLinuxRead\n");
		return;
	}

	if(!qebek_bp_add(LinuxWrite, 0, 0, preLinuxWrite, NULL))
	{
		fprintf(stderr, "qebek_hook_syscall: failed to insert break point for preLinuxWrite\n");
		return;
	}
...
//FIXME: old version
//void qebek_hook_syscall_old(CPUState *env)
//{
